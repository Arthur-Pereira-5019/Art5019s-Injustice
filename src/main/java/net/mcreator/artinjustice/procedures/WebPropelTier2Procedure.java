...
public class WebPropelTier2Procedure {
	public static void execute(Entity entity) {
		...
		entity.setDeltaMovement(new Vec3(((-1) * Math.sin(Math.toRadians(entity.getYRot())) * Math.cos(Math.toRadians(entity.getXRot())) * (boost + 4)), ((-1) * Math.sin(Math.toRadians(entity.getXRot())) * (boost + 4)),
				(Math.cos(Math.toRadians(entity.getYRot())) * Math.cos(Math.toRadians(entity.getXRot())) * (boost + 4))));
	}
}
