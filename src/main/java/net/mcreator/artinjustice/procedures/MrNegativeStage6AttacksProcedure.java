...
public class MrNegativeStage6AttacksProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity target) {
		...
		} else if ((entity instanceof MrNegativeEntity _datEntI ? _datEntI.getEntityData().get(MrNegativeEntity.DATA_attackcycle) : 0) == 18) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator == target) {
						if (!(entityiterator instanceof LivingEntity _livEnt32 && _livEnt32.hasEffect(Art5019injusticeModMobEffects.FEAR.get()))) {
							if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
		...
		} else if ((entity instanceof MrNegativeEntity _datEntI ? _datEntI.getEntityData().get(MrNegativeEntity.DATA_attackcycle) : 0) == 14) {
			{
				final Vec3 _center = new Vec3(x, y, z);
				List<Entity> _entfound = world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(10 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center))).toList();
				for (Entity entityiterator : _entfound) {
					if (entityiterator == target) {
						if (!(entityiterator instanceof LivingEntity _livEnt37 && _livEnt37.hasEffect(MobEffects.DARKNESS))) {
							if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
