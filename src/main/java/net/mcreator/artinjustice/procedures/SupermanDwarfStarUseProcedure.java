...
public class SupermanDwarfStarUseProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
			...
			return;
		{
			double _setval = (entity.getCapability(Art5019injusticeModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new Art5019injusticeModVariables.PlayerVariables())).powermeter
					+ (world.getLevelData().getGameRules().getInt(Art5019injusticeModGameRules.ART_5019_SUPERMAN_NETHER_STAR_ABSORPTION)) * 7;
		...
		if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
			_entity.addEffect(new MobEffectInstance(Art5019injusticeModMobEffects.SUN_DIPPED.get(), 200, 0, false, false));
		...
		if (entity instanceof ServerPlayer _plr9 && _plr9.level() instanceof ServerLevel
				&& _plr9.getAdvancements().getOrStartProgress(_plr9.server.getAdvancements().getAdvancement(new ResourceLocation("art5019injustice:sun_dipped_superman_effect"))).isDone()) {
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
