...
public class VenomOrganicWebbingProcedure {
		...
		if (!entity.isShiftKeyDown()) {
				...
				if (!projectileLevel.isClientSide()) {
					Projectile _entityToSpawn = new Object() {
						public Projectile getArrow(Level level, Entity shooter, float damage, int knockback) {
							AbstractArrow entityToSpawn = new VenomTier1ProjectileEntity(Art5019injusticeModEntities.VENOM_TIER_1_PROJECTILE.get(), level);
			...
			}
			{
				...
				if (!projectileLevel.isClientSide()) {
					Projectile _entityToSpawn = new Object() {
						public Projectile getArrow(Level level, Entity shooter, float damage, int knockback) {
							AbstractArrow entityToSpawn = new VenomWebbingGenericProjectileEntity(Art5019injusticeModEntities.VENOM_WEBBING_GENERIC_PROJECTILE.get(), level);
							entityToSpawn.setOwner(shooter);
							entityToSpawn.setBaseDamage(damage);
							entityToSpawn.setKnockback(knockback);
							entityToSpawn.setSilent(true);
