...
public class TWebShooterTier1RightclickedProcedure {
		...
		if (itemstack.getOrCreateTag().getDouble("cadency") == 0) {
			if (itemstack.getOrCreateTag().getDouble("mode") == 0) {
				...
				if (itemstack.getOrCreateTag().getDouble("webFluid") >= 3) {
					...
					itemstack.getOrCreateTag().putDouble("cadency", 10);
					{
						Entity _shootFrom = entity;
						Level projectileLevel = _shootFrom.level();
						if (!projectileLevel.isClientSide()) {
					...
					if (entity.level().clip(new ClipContext(entity.getEyePosition(1f), entity.getEyePosition(1f).add(entity.getViewVector(1f).scale((40 + boost))), ClipContext.Block.OUTLINE, ClipContext.Fluid.NONE, entity))
