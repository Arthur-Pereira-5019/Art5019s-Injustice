...
public class MrNegativeStage5AttacksProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity target) {
		...
		if ((entity instanceof MrNegativeEntity _datEntI ? _datEntI.getEntityData().get(MrNegativeEntity.DATA_attackcycle) : 0) == 7
		...
		} else if ((entity instanceof MrNegativeEntity _datEntI ? _datEntI.getEntityData().get(MrNegativeEntity.DATA_attackcycle) : 0) == 14) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator == target) {
						if (!(entityiterator instanceof LivingEntity _livEnt39 && _livEnt39.hasEffect(MobEffects.DARKNESS))) {
							if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
								_entity.addEffect(new MobEffectInstance(MobEffects.DARKNESS, 1200, 0, false, false));
							if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
								_entity.addEffect(new MobEffectInstance(MobEffects.BLINDNESS, 1200, 0, false, false));
						}
					}
				}
			}
